...
export async function PATCH(req: NextRequest) {
  try {
    ...
    await redis.hset(`code:${code}`, {
      boundDeviceHash: '',
      bindResetCount: String(record.bindResetCount + 1),
      bindResetAt: new Date().toISOString(),
    });

    return NextResponse.json({
      ok: true,
      message: `Device binding for ${code} has been reset`,
      bindResetCount: record.bindResetCount + 1,
    });
  } catch (err) {
    console.error('[/api/admin/reset-binding] Error:', err);

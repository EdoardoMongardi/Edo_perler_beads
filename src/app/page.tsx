...
export default function Home() {
  ...
  const processFile = async (file: File) => {
    // --- Quota check: reserve a generation slot before processing ---
    if (quota.isRedeemed) {
      const reserveResult = await quota.reserve();
      if (!reserveResult.ok) {
        if (reserveResult.error.includes('exhausted') || reserveResult.error.includes('Quota')) {
      ...
      // Store reservationId for commit/cancel
      ((window as unknown) as Record<string, unknown>).__currentReservationId = reserveResult.reservationId;
    } else if (!quota.loading) {
      // No code redeemed – block generation
      setQuotaModalOpen(true);
      return;
    }
    ...
    if (fileExtension === 'csv') {
      ...
      importCsvData(file)
        .then(({ mappedPixelData, gridDimensions }) => {
          ...
          const rid = ((window as unknown) as Record<string, unknown>).__currentReservationId as string | undefined;
          if (rid) { quota.commit(rid); ((window as unknown) as Record<string, unknown>).__currentReservationId = undefined; }
        })
        .catch(error => {
          console.error('CSV导入失败:', error);
          alert(`CSV导入失败：${error.message}`);
          // Cancel quota reservation on failure
          const rid = ((window as unknown) as Record<string, unknown>).__currentReservationId as string | undefined;
          if (rid) { quota.cancel(rid); ((window as unknown) as Record<string, unknown>).__currentReservationId = undefined; }
        });
    } else {
      // 处理图片文件
  ...
  // Domain redirect removed – this deployment is self-hosted
  ...
  return (
    ...
    <div className="min-h-screen p-4 sm:p-6 flex flex-col items-center bg-gradient-to-b from-pink-50/40 to-white dark:from-gray-800 dark:to-gray-900 font-[family-name:var(--font-geist-sans)] overflow-x-hidden">
      ...
      <header className="w-full md:max-w-4xl text-center mt-6 mb-8 sm:mt-8 sm:mb-10 relative overflow-hidden">
        ...
        <div className="relative z-10 py-8">
          ...
          {quota.isRedeemed && quota.remaining !== null && (
            <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 dark:bg-gray-800/80 border border-pink-200 dark:border-pink-700 shadow-sm">
              <span className="text-xs text-gray-500 dark:text-gray-400">Code: {quota.maskedCode}</span>
              <span className="text-xs text-gray-300 dark:text-gray-600">|</span>
              <span className={`text-sm font-bold ${quota.remaining > 0 ? 'text-pink-500' : 'text-red-500'}`}>
                {quota.remaining}
              </span>
              <span className="text-xs text-gray-400">/ {quota.quotaTotal} left</span>
            </div>
          )}
          {!quota.isRedeemed && !quota.loading && (
            <div className="mt-4 flex flex-col items-center gap-2">
              <p className="text-xs text-gray-500 dark:text-gray-400">Enter a redemption code to start</p>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="px-3 py-1.5 border border-gray-200 dark:border-gray-600 rounded-lg text-sm font-mono tracking-widest text-center focus:outline-none focus:ring-2 focus:ring-pink-300 w-36 bg-white dark:bg-gray-800"
                  maxLength={9}
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      quota.redeem(redeemInput).then((r) => { if (!r.ok) setRedeemError(r.error); else setRedeemError(''); });
                    }
                  }}
                />
                <button
                  onClick={() => quota.redeem(redeemInput).then((r) => { if (!r.ok) setRedeemError(r.error); else setRedeemError(''); })}
                  className="px-4 py-1.5 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-sm font-semibold rounded-lg shadow hover:shadow-md transition-all"
                >
                  Redeem
                </button>
              </div>
      ...
      {/* Footer */}
      <footer className="w-full md:max-w-4xl mt-10 mb-6 py-6 text-center text-xs sm:text-sm text-gray-500 dark:text-gray-400 border-t border-pink-100 dark:border-gray-700 bg-gradient-to-b from-white to-pink-50/50 dark:from-gray-900 dark:to-gray-800/50 rounded-lg shadow-inner">
        <p className="font-medium text-gray-600 dark:text-gray-300">
          PerlerBeads &copy; {new Date().getFullYear()}
        </p>
      </footer>
      ...
      {quotaModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 max-w-sm mx-4 text-center">
            <div className="inline-flex items-center justify-center w-14 h-14 bg-pink-100 rounded-full mb-4">
              <svg className="w-7 h-7 text-pink-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" />
              </svg>
            </div>
            <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2">
              {quota.isRedeemed ? 'Generations Used Up' : 'Redemption Code Required'}
            </h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              {quota.isRedeemed
                ? 'You have used all your generations. Please purchase a new code to continue.'
                : 'You need a valid redemption code to generate images. Please enter your code or visit the redeem page.'}
            </p>
            {!quota.isRedeemed && (
              <div className="flex flex-col gap-2 mb-4">
                <input
                  ...
                  onChange={(e) => setRedeemInput(e.target.value.toUpperCase())}
                  placeholder="XXXX-XXXX"
                  className="px-3 py-2 border border-gray-200 rounded-xl text-center font-mono tracking-widest focus:outline-none focus:ring-2 focus:ring-pink-300"
                  maxLength={9}
                />
                <button
                  onClick={async () => {
                    const r = await quota.redeem(redeemInput);
                    if (r.ok) { setQuotaModalOpen(false); setRedeemError(''); }
                    else setRedeemError(r.error);
                  }}
                  className="px-4 py-2 bg-gradient-to-r from-pink-500 to-rose-500 text-white font-semibold rounded-xl"
                >
                  Redeem
                </button>
                {redeemError && <p className="text-xs text-red-500">{redeemError}</p>}
